...
function aiAssist() {
  const q = el("assistInput").value.trim();
  const ds = state.project.dataset.stats?.tokens || 0;
  const run = state.project.runs[0];
  const advice = [];
  advice.push(state.device.webgpu ? "WebGPU erkannt: nutze tiny/small für stabile Browser-Runs." : "Kein WebGPU: nutze tiny + kurze Steps.");
  if (ds < 5000) advice.push("Dataset erhöhen: mehr Web/Textquellen sammeln oder synthetic generation nutzen.");
  if (run?.state === "overfit") advice.push("Overfit erkannt: mehr Validation, kleineres LR, mehr Datenvielfalt.");
  if (run?.state === "stagnating") advice.push("Stagnation: LR leicht erhöhen oder seq length variieren.");
  advice.push(`Assist Antwort auf: "${q}"`);
